...
def cleanup_directory_by_size(db_path: str, target_path: str, max_bytes: int, max_file_age_days: int, dry_run=False) -> dict:
    ...
    try:
        with sqlite3.connect(db_path, timeout=300) as conn:
            ...
            # 1. Delete files that are too old
            ...
            # 2. Calculate total size of REMAINING (new) files
            ...
            # 3. Check if remaining size is still over budget
            ...
            # 4. Get (new) files, sorted by OLDEST first, and delete them
